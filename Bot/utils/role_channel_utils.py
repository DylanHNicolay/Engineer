...
async def disable_optional_cogs(bot, guild_id: int, except_cogs=None):
    """
    Disable all optional cogs except those specified in except_cogs.
    
    Args:
        bot: The Discord bot instance
        guild_id: The guild ID where cogs should be disabled
        except_cogs: List of cog names that should not be disabled
    """
    if except_cogs is None:
        except_cogs = ["RoleChannelListener"]
    
    # Currently, there are no other optional cogs to disable besides "Setup"
    # This function serves as a placeholder for future expansion
    logger.info(f"Called disable_optional_cogs for guild {guild_id}, keeping {except_cogs}")
    
    # In the future, this function would iterate through loaded cogs
    # and disable those not in the except_cogs list
    pass
